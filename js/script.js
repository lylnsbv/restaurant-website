...
$(document).on('ready', function() {
    ...
    // Send mail from contact form to company
    ...
    $("form[name='contact-form']").on('submit', function(e) {
        ...
        $(this).find(".send_message").attr('value', 'ОТПРАВЛЯЕТСЯ...');
    ...
    // Send reservation mail to company
    $("#reservation_alert").modal("hide");
    $("form[name='table-form']").on('submit', function(e) {

        e.preventDefault();
        var url = "reservation.php";
        var thisForm = $(this);
        var btnValue = $(this).find("#closebooktable").attr('value');
        $(this).find("#closebooktable").attr('value', 'БРОНИРУЕТСЯ...');
        $(this).find("#closebooktable").prop('disabled', true);
        ...
        $.ajax({
            ...
            data: thisForm.serialize(), // serializes the form's elements.
            success: function(data) {
                $("#reservation_alert").html(data);
                thisForm.trigger("reset");
                thisForm.find(".select-dropbox").val("").trigger('change');
                thisForm.find("#closebooktable").attr('value', btnValue);
                thisForm.find("#closebooktable").prop('disabled', false);
                $('#booktable').modal('hide');
                $("#reservation_alert").modal("show");
            }
        });
